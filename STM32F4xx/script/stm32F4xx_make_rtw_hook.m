function stm32F4xx_make_rtw_hook(hookMethod,modelName,rtwroot,templateMakefile,buildOpts,buildArgs)
% stm32F4xxMAKERTWHOOK supports build process
%
% $ stm32F4xx_make_rtw_hook.m 2009-05-14 dlange $
%
...
switch hookMethod
	...
	case 'before_make'
		...
		%disp(sprintf(['### before_make Set environement variable \n']));
		...
		%disp(sprintf(['### before_make Add makefile for PIL \n']));
		...
		%disp(sprintf(['### before_make Add additional sources to makefile 1\n']));
		...
		%disp(sprintf(['### before_make Add additional sources to makefile 2-1\n']));
		...
		disp(sprintf(['### before_make Add additional sources to makefile 2-2\n']));
		...
		if (isempty(Stm32LibFiles2Build))
			...
			disp(sprintf(['### before_make Add additional sources to makefile 2-3\n']));
			listfiles = {Stm32LibFiles(logical(reshape(Stm32LibFiles2Build,1,numel(Stm32LibFiles)))).name};
			disp(sprintf(['### before_make Add additional sources to makefile 2-4\n']));
			for index = listfiles
				sourceFiles = [sourceFiles,char(index),' '];
			end
			disp(sprintf(['### before_make Add additional sources to makefile 2-5\n']));
			clear listfiles index
		end
		
		disp(sprintf(['### before_make Add additional sources to makefile 3\n']));
		sourceFiles = [sourceFiles,'stm32f4xx_flash.c ',...
		               'stm32f4xx_pwr.c ' ];
		itFile      = ['stm32f4xx_it.c '];			  
		
		disp(sprintf(['### before_make Add additional sources to makefile 4\n']));
		
		stmSeries = get_param(modelName,'stm32Series');
		startup_file = ['startup_',lower(stmSeries),'.s'];
		switch toolChainSel
			case 'IAR'
			...
			%startupFile = 'startup_stm32f4xx.s';
			startupFile = startup_file;
			%startupFile = [Stm32RootPath,'\lib\CMSIS\Device\ST\STM32F4xx\Source\Templates\arm\startup_stm32f4xx.s'];
			case 'ATOLLIC'
			%startupFile = 'startup_stm32f4xx.s';
			startupFile = startup_file;
			%startupFile = [Stm32RootPath,'\lib\CMSIS\Device\ST\STM32F4xx\Source\Templates\TrueSTUDIO\startup_stm32f4xx.s'];
        end
        
		disp(sprintf(['### before_make Add additional sources to makefile 5\n']));
		...
		if (strcmp(get_param(modelName,'DownloadApplication'),'on') && ...
			...
			switch toolChainSel
				case 'IAR'
                    ...
                    stmdevice = get_param(modelName,'stm32Device');
					...
					[' --semihosting --drv_verify_download  --drv_communication=USB0 ',' --device=',stmdevice]];
